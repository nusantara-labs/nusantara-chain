...
fn main() {
    ...
    for (key, value) in &values {
        let env_key = format!("NUSA_RPC_{}", key.to_uppercase());
        println!("cargo:rustc-env={env_key}={value}");
    }
}

/// Simple flat TOML parser matching the project-wide pattern.
/// Returns `SECTION_KEY` pairs as strings; numeric values have underscores stripped.
